...
void WWTMVAClassification( TString myMethodList = "" ) 
{
   ...
   // factory->AddVariable("SumET := event_met_pfsumet", 'F');
   //factory->AddVariable("pfmetSign := event_met_pfmetsignificance", 'F');

   // dPhi stuff
   factory->AddVariable("dphiLJ1 := abs(abs(abs(W_muon_phi-JetPFCor_Phi[0])-TMath::Pi())-TMath::Pi())", 'F');
   factory->AddVariable("dphiLJ2 := abs(abs(abs(W_muon_phi-JetPFCor_Phi[1])-TMath::Pi())-TMath::Pi())", 'F');
   // factory->AddVariable("dphiLMet := abs(abs(abs(W_muon_phi-event_met_pfmetPhi)-TMath::Pi())-TMath::Pi())", 'F');
   factory->AddVariable("dphiJ1J2 := abs(abs(abs(JetPFCor_Phi[0]-JetPFCor_Phi[1])-TMath::Pi())-TMath::Pi())", 'F');
   factory->AddVariable("fitChi2 := fit_chi2", 'F');
   factory->AddVariable("LepPt := W_muon_pt", 'F');
   factory->AddVariable("LepEta := W_muon_eta", 'F');
   factory->AddVariable("LepCharge := W_muon_charge", 'F');


   // jet variables
   factory->AddVariable("J1Pt := JetPFCor_Pt[0]", 'F');
   // factory->AddVariable("J1Eta := JetPFCor_Eta[0]", 'F');
   // factory->AddVariable("J1Mass := JetPFCor_Mass[0]", 'F');
   factory->AddVariable("J1QGL := JetPFCor_QGLikelihood[0]", 'F');

   factory->AddVariable("J2Pt := JetPFCor_Pt[1]", 'F');
   //  factory->AddVariable("J2Eta := JetPFCor_Eta[1]", 'F');
   // factory->AddVariable("J2Mass := JetPFCor_Mass[1]", 'F');
   factory->AddVariable("J2QGL := JetPFCor_QGLikelihood[1]", 'F');

   //factory->AddVariable("DijetPt := sqrt(JetPFCor_Pt[0]**2+JetPFCor_Pt[1]**2+2*JetPFCor_Pt[0]*JetPFCor_Pt[1]*cos(JetPFCor_Phi[0]-JetPFCor_Phi[1]))", 'F');
   //factory->AddVariable("Pt2OverMjj := JetPFCor_Pt[1]/Mass2j_PFCor", 'F');
   // factory->AddVariable("Jacobian := W_pt/MassV2j_PFCor", 'F');

   factory->AddVariable("detaJ1J2 := JetPFCor_Eta[0]-JetPFCor_Eta[1]", 'F');
   factory->AddVariable("cosW2j := cosJacksonAngleV2j_PFCor", 'F');
   // factory->AddVariable("cosLnu := cosThetaLnu_PFCor", 'F');
   factory->AddVariable("cosJJ := cosThetaJJ_PFCor", 'F');
  ...
  // factory->AddVariable("QGL := JetPFCor_QGLikelihood[0]*JetPFCor_QGLikelihood[1]", 'F');
  // factory->AddVariable("ColorPull := WJetsPullPFCor", 'F');
  ...
  // You can add so-called "Spectator variables", which are not used in the MVA training, 
   ...
   factory->AddSpectator("fit_mjj := fit_mjj", "F");
   ...
   // read training and test data
    ...
    TFile *input2 = TFile::Open( "/uscms_data/d2/kalanand/WjjTrees/Full2011DataFall11MC/ReducedTree/RD_mu_WpJ_CMSSW428.root" );
