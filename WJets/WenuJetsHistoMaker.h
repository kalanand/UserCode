...
#ifndef WenuJetsHistoMaker_h
#define WenuJetsHistoMaker_h

#include <TROOT.h>
#include <TChain.h>
#include <TFile.h>

class WenuJetsHistoMaker {
public :
   ...
   WenuJetsHistoMaker(TTree *tree=0);
   virtual ~WenuJetsHistoMaker();
   ...
   virtual double Phi_0_2pi(double x);
   virtual double Phi_mpi_pi(double x);
   virtual double dPhi(double phi1,double phi2);
   virtual double radius(double eta1, double phi1,double eta2, double phi2);
   virtual void     myana();
};

#endif

#ifdef WenuJetsHistoMaker_cxx
WenuJetsHistoMaker::WenuJetsHistoMaker(TTree *tree)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("Data_WenuJets_2011_178invpb_May6JSON.root");
      if (!f) {
         f = new TFile("Data_WenuJets_2011_178invpb_May6JSON.root");
      }
...
WenuJetsHistoMaker::~WenuJetsHistoMaker()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t WenuJetsHistoMaker::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t WenuJetsHistoMaker::LoadTree(Long64_t entry)
{
...
void WenuJetsHistoMaker::Init(TTree *tree)
{
...
Bool_t WenuJetsHistoMaker::Notify()
{
...
void WenuJetsHistoMaker::Show(Long64_t entry)
{
...
Int_t WenuJetsHistoMaker::Cut(Long64_t entry)
{
...
double WenuJetsHistoMaker::Phi_0_2pi(double x) {
  while (x >= 2*M_PI) x -= 2*M_PI;
  while (x <     0.)  x += 2*M_PI;
  return x;
}

double WenuJetsHistoMaker::Phi_mpi_pi(double x) {
   while (x >= M_PI) x -= 2*M_PI;
   while (x < -M_PI) x += 2*M_PI;
   return x;
}

double WenuJetsHistoMaker::dPhi(double phi1,double phi2){
   phi1=Phi_0_2pi(phi1);
   phi2=Phi_0_2pi(phi2);
   return Phi_mpi_pi(phi1-phi2);
}

double WenuJetsHistoMaker::radius(double eta1, double phi1,double eta2, double phi2){
...
#endif // #ifdef WenuJetsHistoMaker_cxx
