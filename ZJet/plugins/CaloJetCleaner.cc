...
class CaloJetCleaner : public edm::EDProducer
{
public:
  // construction/destruction
  CaloJetCleaner(const edm::ParameterSet& iConfig);
  virtual ~CaloJetCleaner();
...
CaloJetCleaner::CaloJetCleaner(const edm::ParameterSet& iConfig)
...
//______________________________________________________________________________
void CaloJetCleaner::produce(edm::Event& iEvent,const edm::EventSetup& iSetup)
{
  auto_ptr<reco::CaloJetCollection>
    cleanJets(new reco::CaloJetCollection);
  ...
  for (unsigned int iSrc=0;iSrc<srcObjects_.size();iSrc++) {
    ...
    for (unsigned int iJet=0;iJet<jets->size();iJet++) {
      const reco::CaloJet& jet = jets->at(iJet);
