...
void plot_ZJetBalance( int bin, TH1& responseHistGen, 
  ...
  Float_t JetRecoCorrectedPt[NUM_JET_MAX];
  ...
  if(usingCorrectedCaloJetPt == true) {
    t->SetBranchAddress("JetRecoCorrectedPt",  JetRecoCorrectedPt);
    t->SetBranchStatus("JetRecoCorrectedPt",     1);
  }
  ...
  for (Long64_t entry =0; entry < t->GetEntries(); entry++) {
    ...
    float leadGenJetPt=JetGenPt[0]; 
    float secondGenJetPt=JetGenPt[0]; 
    ...
    if(usingCorrectedCaloJetPt == false) {
      leadRecoIndex = 0; 
      secondRecoIndex = 1;
      leadRecoJetPt   =  JetRecoPt[0];
      secondRecoJetPt =  JetRecoPt[1];
    }
    else {
      FindLeadIndex(JetRecoCorrectedPt, leadRecoIndex, secondRecoIndex);
      leadRecoJetPt   = JetRecoCorrectedPt[leadRecoIndex];
      secondRecoJetPt = JetRecoCorrectedPt[secondRecoIndex];
    }
    ...
    double dPhiReco = dPhi(JetRecoPhi[leadRecoIndex], Z_Phi);




//     if( fabs(leadRecoJetEta)>1.3 || fabs(dPhiReco)<2.94 || 
// 	secondRecoJetPt/leadRecoJetPt>0.2 ) continue;


//     if( fabs(leadRecoJetEta)>1.3 || fabs(dPhiReco)<2.94 ) continue;

//     std::cout<<"************ Point 2 "<< std::endl;


    double ptRatioGen  = leadGenJetPt/Z_PtGen;
    double ptRatioReco = leadRecoJetPt/Z_Pt;
