...
const bool usingCorrectedCaloJetPt = true;
...
void plot_ZJetBalance( int bin, TH1& responseHistGen, 
  ...
  for (Long64_t entry =0; entry < t->GetEntries(); entry++) {
    ...
    int leadGenIndex=0, secondGenIndex=0, 
...
void FindLeadIndex(float pT[], float eta[], float phi[], 
		   ...
		   int &lead, int &sec, double correction = 1.0) {
  ...
  for (int i=0; i<NUM_JET_MAX; i++) {
    ...
    float correctedPt = pT[i]* correction;
    if(correctedPt>max) { max = correctedPt; lead = i; }
  }
  ...
  for (int i=0; i<NUM_JET_MAX; i++) {
    ...
    if(radius(eta[i], phi[i], e2eta, e2phi) < 0.3) continue;
    float correctedPt = pT[i]* correction;
    if(correctedPt>max) { max = correctedPt; sec = i; }
  }
}
