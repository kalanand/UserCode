...
mkdir -p include; 
cd include;
if [ ! -h TMVA ]; then ln -s ../src TMVA; fi;
cd ..

# set symbolic links to data file and to rootmaps
cd macros;
if [ ! -h tmva_example.root ]; then ln -s ../execs/data/toy_sigbkg.root tmva_example.root; fi
if [ ! -h tmva_reg_example.root ]; then ln -s ../execs/data/regression_parabola_noweights.root tmva_reg_example.root; fi
