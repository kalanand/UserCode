...
void WWTMVAClassification( TString myMethodList = "" ) 
{
   ...
   // leptonic W
   factory->AddVariable("Wmt := W_mt", 'F');
   //factory->AddVariable("Wpt := W_pt", 'F');
   // factory->AddVariable("pfmet := event_met_pfmet", 'F');
   // factory->AddVariable("pfmetSign := event_met_pfmetsignificance", 'F');
   // factory->AddVariable("calomet := event_met_calomet", 'F');
   // factory->AddVariable("caalometSign := event_met_calometsignificance", 'F');

   // electron Id
   // factory->AddVariable("HoverE := W_electron_hovere", 'F');
   // factory->AddVariable("ecalIso := W_electron_ecaliso/W_electron_et", 'F');
   // factory->AddVariable("hcalIso := W_electron_hcaliso/W_electron_et", 'F');
   factory->AddVariable("et := W_electron_et", 'F');

   // jet variables
   // factory->AddVariable("LeadPt := JetPFCor_Pt[0]", 'F');
   // factory->AddVariable("LeadNeutralEMF := JetPFCor_NeutralEmEnergyFrac[0]", 'F');

   // factory->AddVariable("secondPt := JetPFCor_Pt[1]", 'F');
   // factory->AddVariable("SecondNeutralEMF := JetPFCor_NeutralEmEnergyFrac[1]", 'F');

   factory->AddVariable("detaJ1J2 := JetPFCor_Eta[0]-JetPFCor_Eta[1]", 'F');
   factory->AddVariable("cosThetaStarJ1J2 := tanh(0.5*(JetPFCor_Y[0]-JetPFCor_Y[1]))", 'F');
   factory->AddVariable("dphiJ1J2 := abs(abs(abs(JetPFCor_Phi[0]-JetPFCor_Phi[1])-TMath::Pi())-TMath::Pi())", 'F');
   factory->AddVariable("dphieMet := abs(abs(abs(W_electron_phi-event_met_pfmetPhi)-TMath::Pi())-TMath::Pi())", 'F');
  factory->AddVariable("dphiJ1Met := abs(JetPFCor_dphiMET[0])", 'F');
  factory->AddVariable("dphiJ2Met := abs(JetPFCor_dphiMET[1])", 'F');
  factory->AddVariable("dphiWW := abs(abs(abs(W_phi-atan2((JetPFCor_Py[0]+JetPFCor_Py[1]),(JetPFCor_Px[0]+JetPFCor_Px[1])))-TMath::Pi())-TMath::Pi())");

  factory->AddVariable("etaetaMoment0 := JetPFCor_etaetaMoment[0]", 'F');
  factory->AddVariable("phiphiMoment0 := JetPFCor_phiphiMoment[0]", 'F');
  factory->AddVariable("etaphiMoment0 := JetPFCor_etaphiMoment[0]", 'F');
  factory->AddVariable("maxDistance0 := JetPFCor_maxDistance[0]", 'F');
  factory->AddVariable("Mass0 := JetPFCor_Mass[0]", 'F');
  factory->AddVariable("Response0 := JetPFCor_Response[0]", 'F');
  factory->AddVariable("etaetaMoment1 := JetPFCor_etaetaMoment[1]", 'F');
  factory->AddVariable("phiphiMoment1 := JetPFCor_phiphiMoment[1]", 'F');
  factory->AddVariable("etaphiMoment1 := JetPFCor_etaphiMoment[1]", 'F');
  factory->AddVariable("maxDistance1 := JetPFCor_maxDistance[1]", 'F');
  factory->AddVariable("Mass1 := JetPFCor_Mass[1]", 'F');
  factory->AddVariable("Response1 := JetPFCor_Response[1]", 'F');
  ...
  // You can add so-called "Spectator variables", which are not used in the MVA training, 
   ...
   factory->AddSpectator("mjj := sqrt(JetPFCor_E[0]**2-JetPFCor_Pt[0]**2-JetPFCor_Pz[0]**2+JetPFCor_E[1]**2-JetPFCor_Pt[1]**2-JetPFCor_Pz[1]**2 + 2.*(JetPFCor_E[0]*JetPFCor_E[1]-JetPFCor_Px[0]*JetPFCor_Px[1]-JetPFCor_Py[0]*JetPFCor_Py[1]-JetPFCor_Pz[0]*JetPFCor_Pz[1]))", "F");



   // read training and test data
    TFile *input1 = TFile::Open( "../../WJet/WW_enujj_MC.root" );
    TFile *input2 = TFile::Open( "../../WJet/WJets_MC.root" );
...
//    TCut mycutb = "abs(eta)>1.5"; // for example: TCut mycutb = "abs(var1)<0.5";
  ...
  TCut goodW("W_electron_et>25. && event_met_pfmet>25.");
  TCut twojets("JetPFCor_Pt[0]>20. && JetPFCor_Pt[1]>20. && JetPFCor_Pt[2]<20.0");
  TCut noBJets("numPFCorJetBTags==0");

  TCut mycuts (goodW && twojets && noBJets);
