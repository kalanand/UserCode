...
void Combiner ( char* ZeeJetFile, char* GammaJetFile) {
  ...
  if(plotCase==2) PlotCombinedCorrection( *merged_gr, *new_gr, *new_gr2 );
  if(plotCase==4) PlotConstantDisagreement( *merged_gr );
  if(plotCase==5) PlotLinearDisagreement( *merged_gr );
}
...
void PlotCombinedCorrection( const TGraphErrors& g,  
  ...
  TCanvas *c_Correction3 = new TCanvas("Correction3","Correction3",900,600);
  gStyle->SetOptStat(0000);
  c_Correction3->cd(); 
  gPad->SetLogx();  
  hCorrErr->Draw("E3");
  ...
  C_all->Draw("Lsame");
  TLegend *leg = new TLegend(0.5,0.65,0.89,0.89);
  leg->AddEntry( &g1,"Measurement from Z#rightarrow e^{+}e^{-}+Jet","LP");
  leg->AddEntry( &g2,"Measurement from #gamma+Jet","LP");
...
void PlotConstantDisagreement( const TGraphErrors& g ) {
  ...
  TCanvas *c_Correction4 = new TCanvas("Correction4","Correction4",900,600);
  gStyle->SetOptStat(0000);
  c_Correction4->cd(); 
  gPad->SetLogx();  
...
void PlotLinearDisagreement( const TGraphErrors& g ) {
  ...
  TCanvas *c_Correction5 = new TCanvas("Correction5","Correction5",900,600);
  gStyle->SetOptStat(0000);
  c_Correction5->cd(); 
  gPad->SetLogx();  
